...
import { deleteService, getListService } from 'redux/slices/dashboard/services';
...
const TABLE_HEAD = [
  { id: 'id', label: i18n.t<string>('serviceId'), align: 'left', width: 150 },
  { id: 'name', label: i18n.t<string>('serviceName'), align: 'left', width: 300 },
...
export default function ServiceListPage() {
  ...
  const { serviceList, serviceCount } = useSelector((state) => state.service);
  ...
  const handleDeleteRows = async (selectedRows: string[]) => {
    await dispatch(deleteService(selectedRows));
    await getServiceList({
      pageIndex: DEFAULT_PAGINATION.PAGE_INDEX,
      pageSize: DEFAULT_PAGINATION.PAGE_SIZE,
      eventType: EVENT_TYPE.SERVICE,
  ...
  const handleEditRow = (id: string) => {
    navigate(PATH_DASHBOARD.app.service.edit(id));
  };
  ...
  const getServiceList = async (options: EventParamsType) => {
    setLoading(true);
    try {
      await dispatch(getListService(options));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageWrapper title={t('service')}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={t('listOfService')}
          links={[
          ...
          action={
            <Button
              ...
              to={PATH_DASHBOARD.app.service.new}
              variant="contained"
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            <TableSelectedAction
              ...
              onSelectAllRows={(checked) =>
                onSelectAllRows(
                  checked,
                  serviceList?.map((row: any) => row.id.toString())
                )
              }
              action={
