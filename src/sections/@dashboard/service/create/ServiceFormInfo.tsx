...
import { RHFCheckbox, RHFEditor, RHFTextField, RHFUpload } from 'components/hook-form';
...
const ServiceFormInfo = ({ editValue, setEditValue, methods }: Props) => {
  const { t } = useLocales();
      ...
      <Card sx={{ px: 3, py: 3 }}>
        <Grid container spacing={3} >
          <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
            <RHFTextField name="name" isRequired label={t('nameService')} />
          </Grid>
          <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
...
export default ServiceFormInfo;
